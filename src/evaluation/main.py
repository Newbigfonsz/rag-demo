...
class Evaluator:
    ...
    def run(self):
        ...
        for i, query in enumerate(TEST_CASES, 1):
            ...
            start = time.time()
            response = self.pipeline.query(query, top_k=5)
            latency = (time.time() - start) * 1000
            
            retrieved = [s.source_file for s in response.sources]
            avg_score = sum(response.retrieval_scores) / len(response.retrieval_scores)
            ...
            status = "pass" if avg_score > 0.6 else "warn"
            self.console.print(f"  Score: {avg_score:.3f}, Latency: {latency:.0f}ms")
    ...
    def print_summary(self, results):
        ...
        for r in results:
            style = "green" if r.avg_score > 0.7 else "yellow" if r.avg_score > 0.5 else "red"
            table.add_row(r.query[:35], f"[{style}]{r.avg_score:.3f}[/{style}]", f"{r.latency_ms:.0f}ms")
        
        self.console.print(table)
        
        avg_score = sum(r.avg_score for r in results) / len(results)
        avg_latency = sum(r.latency_ms for r in results) / len(results)
        passed = sum(1 for r in results if r.avg_score > 0.6)
